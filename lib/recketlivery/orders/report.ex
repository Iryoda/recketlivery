defmodule Recketlivery.Orders.Report do
  import Ecto.Query

  alias Recketlivery.{Order, Repo, Item}
  alias Recketlivery.Orders.TotalPrice
  ...
  def parse_line(%Order{user_id: user_id, payment_method: payment_methods, items: items}) do
    total_price = TotalPrice.calculate(items)

    items_string = Enum.map(items, &items_string/1)
    "#{user_id},#{payment_methods},#{items_string}#{total_price}\n"
  end
